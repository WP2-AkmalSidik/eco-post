...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
        ...
        <!-- Form Blog -->
        <form id="blog-form" action="{{ route('post.update', ['slug' => $post->slug]) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            @method('PUT')
            <!-- Featured Image -->
            <div class="mb-6">
                <div id="image-preview" class="w-full h-110 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center cursor-pointer hover:bg-gray-50">
                    <div id="upload-prompt" class="{{ $post->image_path ? 'hidden' : '' }} text-center">
                        <i class="fas fa-image text-3xl text-gray-400 mb-2"></i>
                        <p class="text-gray-500">Click to add featured image</p>
                    </div>
                    <img id="selected-image" class="{{ $post->image_path ? '' : 'hidden' }} w-full h-full object-cover rounded-lg"
                         src="{{ $post->image_path ? asset('storage/' . $post->image_path) : '' }}" alt="Featured image">
                </div>
            ...
            <div class="mb-6 flex flex-col sm:flex-row gap-4">
                <div class="flex-grow">
                    <input type="text" id="post-title" name="title" placeholder="Post Title" required value="{{ old('title', $post->title) }}"
                ...
                <div class="sm:w-1/4">
                    <select id="post-category" name="category" required
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ (old('category', $selectedCategory ? $selectedCategory->id : '')) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <textarea id="hidden-content" name="content" class="hidden">{{ old('content', $post->body) }}</textarea>
